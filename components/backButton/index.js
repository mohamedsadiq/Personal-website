// ... existing imports and code up to line 3 unchanged ...

import { useState } from 'react';
import { useRouter } from 'next/router';  // Add this import for dynamic back navigation

const BackButton = ({ 
  href = '/sparks',  // Retained for compatibility, but unused for intelligent back
  className = '',
  title = '' // Keep for backward compatibility
}) => {
    const [copied, setCopied] = useState(false);
    const router = useRouter();  // Hook for router access

    // New handler for intelligent back navigation
    const handleBack = () => {
      router.back();
    };

    // Existing commented-out handler preserved
    // const handleCopyClick = () => {
    //     navigator.clipboard.writeText(link).then(() => {
    //         setCopied(true);
    //         setTimeout(() => {
    //             setCopied(false);
    //         }, 1000);
    //     });
    // };

    return (
        <div className="nav_work mt-24">
            <div>
                {/* Updated to div with onClick/onKeyDown for dynamic back; preserves all classes and structure */}
                <div 
                  className={`${className} inline-block cursor-pointer rounded-[50%]`} 
                  onClick={handleBack}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') {
                      handleBack();
                      e.preventDefault();
                    }
                  }}
                  role="button"
                  tabIndex={0}
                  aria-label="Go back to previous page"
                >
                    <div className="go_back group">
                        <svg width="18" height="18" viewBox="0 0 93 72" fill="none" xmlns="http://www.w3.org/2000/svg" className="transition-colors duration-100 group-hover:fill-black fill-[#ababab]">
                            <path d="M93.0001 42.7733C92.9803 50.3076 89.9785 57.5277 84.6509 62.8552C79.3234 68.1828 72.1033 71.1845 64.5691 71.2043H34.1301C33.1427 71.2043 32.1957 70.8121 31.4975 70.1139C30.7993 69.4157 30.4071 68.4687 30.4071 67.4813C30.4071 66.4939 30.7993 65.547 31.4975 64.8488C32.1957 64.1506 33.1427 63.7583 34.1301 63.7583H64.5691C70.1097 63.7201 75.4103 61.4923 79.3147 57.5609C83.219 53.6295 85.4102 48.3136 85.4102 42.7728C85.4102 37.2321 83.219 31.9162 79.3147 27.9848C75.4103 24.0534 70.1097 21.8256 64.5691 21.7873H12.7091L20.7001 29.7743C21.3906 30.4741 21.7763 31.4186 21.773 32.4017C21.7697 33.3848 21.3777 34.3267 20.6826 35.0218C19.9874 35.717 19.0455 36.109 18.0624 36.1123C17.0793 36.1155 16.1348 35.7298 15.4351 35.0393L1.08906 20.6973C0.391689 19.9989 0 19.0523 0 18.0653C0 17.0784 0.391689 16.1317 1.08906 15.4333L15.4301 1.09233C16.1281 0.393486 17.0752 0.000563214 18.0629 6.05009e-07C19.0507 -0.000562004 19.9982 0.391281 20.6971 1.08933C21.3959 1.78738 21.7888 2.73446 21.7894 3.72221C21.7899 4.70996 21.3981 5.65749 20.7001 6.35633L12.7141 14.3423H64.5741C72.1075 14.3634 79.3262 17.3658 84.6527 22.6932C89.9792 28.0206 92.9803 35.2399 93.0001 42.7733Z"/>
                        </svg>
                    </div>
                </div>
            </div>

            {/* Existing commented-out copy section preserved */}
            {/* <div className="rel_parent pointer">
                <div className='linkInfoProject' >
                    {copied ? 'copied' : 'copy url'} 
                </div>
                <div href={link ? link : "Title"} passHref onClick={handleCopyClick}>
                    <svg className="center_right" width="30" height="30" viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fillRule="evenodd" clipRule="evenodd" d="M18.3291 11.1014C19.2167 10.4985 20.3389 10.4867 21.1727 11.0342C21.362 11.1586 21.5642 11.3559 22.1048 11.8964C22.6453 12.437 22.8426 12.6392 22.967 12.8285C23.5145 13.6623 23.5027 14.7845 22.8998 15.6721C22.7621 15.8747 22.5491 16.0919 21.9888 16.6522L21.1768 17.4642C20.9206 17.7205 20.9206 18.1359 21.1768 18.3922C21.4331 18.6484 21.8485 18.6484 22.1048 18.3922L22.9167 17.5802L22.9731 17.5239L22.9731 17.5239C23.4582 17.0389 23.7686 16.7286 23.9853 16.4096C24.8714 15.1052 24.9172 13.4074 24.0639 12.1081C23.8558 11.7912 23.5525 11.4881 23.0913 11.027L23.0327 10.9685L22.9742 10.9099C22.5131 10.4487 22.21 10.1454 21.8931 9.93729C20.5938 9.08402 18.896 9.12977 17.5916 10.0159C17.2726 10.2326 16.9623 10.543 16.4773 11.0281L16.421 11.0845L15.609 11.8964C15.3528 12.1527 15.3528 12.5681 15.609 12.8244C15.8653 13.0806 16.2807 13.0806 16.537 12.8244L17.349 12.0124C17.9093 11.4521 18.1265 11.2391 18.3291 11.1014ZM12.8251 16.5363C13.0814 16.28 13.0814 15.8645 12.8251 15.6083C12.5689 15.352 12.1534 15.352 11.8972 15.6083L11.0852 16.4203L11.0289 16.4766L11.0289 16.4766C10.5438 16.9615 10.2334 17.2719 10.0166 17.5909C9.1305 18.8953 9.08476 20.5931 9.93802 21.8924C10.1461 22.2092 10.4494 22.5124 10.9106 22.9734L10.9692 23.032L11.0278 23.0906L11.0278 23.0906C11.4888 23.5518 11.792 23.8551 12.1088 24.0632C13.4081 24.9165 15.1059 24.8707 16.4103 23.9846C16.7294 23.7679 17.0397 23.4574 17.5246 22.9723L17.5247 22.9723L17.5809 22.916L18.3929 22.104C18.6492 21.8478 18.6492 21.4323 18.3929 21.1761C18.1367 20.9198 17.7212 20.9198 17.465 21.1761L16.653 21.988C16.0926 22.5484 15.8755 22.7614 15.6729 22.899C14.7853 23.502 13.663 23.5138 12.8292 22.9662C12.6399 22.8419 12.4377 22.6446 11.8972 22.104C11.3566 21.5635 11.1593 21.3613 11.035 21.172C10.4874 20.3382 10.4992 19.2159 11.1022 18.3283C11.2398 18.1257 11.4528 17.9086 12.0132 17.3482L12.8251 16.5363ZM19.7851 15.1439C20.0413 14.8876 20.0413 14.4721 19.7851 14.2159C19.5288 13.9596 19.1133 13.9596 18.8571 14.2159L14.2173 18.8557C13.961 19.112 13.961 19.5274 14.2173 19.7837C14.4735 20.0399 14.889 20.0399 15.1452 19.7837L19.7851 15.1439Z" fill="white"/>
                        <circle cx="17" cy="17" r="16.2273" stroke="white" strokeWidth="1.54545"/>
                    </svg>
                </div>
            </div> */}
        </div>
    );
};

export default BackButton;

// ... rest unchanged (line 47-48)